                    ...
                    <div class="page">
                        <div>
                            @if(!$receives->onFirstPage())
                                <a class="prev"
                                   href="{{$receives->previousPageUrl()}}&openid={{$openid}}&status={{$status}}">&lt;&lt;</a>
                                @if($receives->currentPage()-1>1)
                                    <a class="num"
                                       href="{{$receives->url(1)}}&openid={{$openid}}&status={{$status}}">1</a>
                                @endif
                                <a class="num"
                                   href="{{$receives->previousPageUrl()}}&openid={{$openid}}&status={{$status}}">{{$receives->currentPage()-1}}</a>
                            @else
                            ...
                            @if($receives->currentPage()!=$receives->lastPage())
                                <a class="num"
                                   href="{{$receives->nextPageUrl()}}&openid={{$openid}}&status={{$status}}">{{$receives->currentPage()+1}}</a>
                                @if($receives->lastPage()-$receives->currentPage()>1)
                                    <a class="num"
                                       href="{{$receives->url($receives->lastPage())}}&openid={{$openid}}&status={{$status}}">{{$receives->lastPage()}}</a>
                                @endif
                                <a class="next"
                                   href="{{$receives->nextPageUrl()}}&openid={{$openid}}&status={{$status}}">&gt;&gt;</a>
                            @else
